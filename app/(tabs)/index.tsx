...
export default function TodayScreen() {
  ...
  return <Screen>
    <View style={styles.top}><View><Text style={styles.eyebrow}>{t("today.eyebrow")}</Text><Text style={styles.title}>{selectedDog.name}</Text><Text style={styles.subtle}>{hasDog ? `${ageInMonths(selectedDog)} månader · ${selectedDog.breed ?? t("dog.unknown")}` : t("onboarding.body")}</Text></View><Avatar emoji={selectedDog.avatar} size={64} /></View>
    ...
    {hasDog ? <View style={styles.routineRow}><Pill onPress={() => addRoutine("wake")}>☀️ {t("today.wake")}</Pill><Pill onPress={() => addRoutine("meal")}>🥣 {t("today.meal")}</Pill><Pill onPress={() => addRoutine("play")}>🧸 {t("today.play")}</Pill></View> : null}
    <SectionTitle title={t("today.last")} action={t("nav.log")} onAction={() => setLogVisible(true)} />
