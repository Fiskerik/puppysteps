...
type LogEntry = { type: "checkIn"; id: string; occurredAt: string; dogName: string; checkIn: EditableCheckIn } | { type: "routine"; id: string; occurredAt: string; dogName: string; event: RoutineEvent };
...
export default function LogScreen() {
  ...
  const dogEvents = useMemo(() => snapshot.eliminations.filter((event) => dogFilter === "all" || event.dogId === dogFilter), [dogFilter, snapshot.eliminations]);
  const entries = useMemo<LogEntry[]>(() => {
    const dogName = (dogId: string) => snapshot.dogs.find((dog) => dog.id === dogId)?.name ?? t("dog.unknown");
    const checkIns: LogEntry[] = snapshot.checkIns.filter((checkIn) => dogFilter === "all" || checkIn.dogId === dogFilter).map((checkIn) => ({ type: "checkIn", id: checkIn.id, occurredAt: checkIn.occurredAt, dogName: dogName(checkIn.dogId), checkIn: { checkIn, events: dogEvents.filter((event) => event.checkInId === checkIn.id) } }));
    const routines: LogEntry[] = snapshot.routineEvents.filter((event) => dogFilter === "all" || event.dogId === dogFilter).map((event) => ({ type: "routine", id: event.id, occurredAt: event.occurredAt, dogName: dogName(event.dogId), event }));
  ...
  const renderEntry = (entry: LogEntry) => {
    ...
    return <Pressable key={entry.id} accessibilityRole="button" accessibilityLabel={`${entry.dogName}, ${label}, ${dateTime(entry.occurredAt, snapshot.settings.locale)}`} onPress={() => isCheckIn ? openCheckIn(entry.checkIn) : openRoutine(entry.event)} onLongPress={() => isCheckIn ? openCheckIn(entry.checkIn) : openRoutine(entry.event)} delayLongPress={350} style={styles.item}>
      <View style={styles.iconGroup}>{icons.map((icon, index) => <Text key={`${entry.id}-${index}`} style={styles.itemIcon}>{icon}</Text>)}</View>
      <View style={styles.itemCopy}><Text style={styles.itemTitle}>{label}</Text><Text style={styles.itemMeta}>{dogFilter === "all" && snapshot.dogs.length > 1 ? `${entry.dogName} · ` : ""}{dateTime(entry.occurredAt, snapshot.settings.locale)}</Text></View>
  ...
  return <Screen>
    <View style={styles.header}><View><Text style={styles.eyebrow}>{t("nav.log")}</Text><Text style={styles.title}>{dogFilter === "all" && snapshot.dogs.length > 1 ? t("common.allDogs") : selectedDog.name}</Text><Text style={styles.subtle}>{t("today.subtle")}</Text></View></View>
    <View style={styles.dogRow}><Pill active={dogFilter === "all"} onPress={() => setDogFilter("all")}>{t("common.allDogs")}</Pill>{snapshot.dogs.map((dog) => <Pill key={dog.id} active={dog.id === dogFilter} onPress={() => { setDogFilter(dog.id); setSelectedDogId(dog.id); }}>{dog.name}</Pill>)}</View>
